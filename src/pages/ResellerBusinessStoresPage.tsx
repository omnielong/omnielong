...
const ResellerBusinessStoresPage: React.FC = () => {
  ...
  const { stores, businesses, currentUser, login } = useStore();
  const [searchQuery, setSearchQuery] = useState('');
  const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'inactive'>('all');

  // Verifica che sia un reseller
  useEffect(() => {
  ...
  // Filtra store del business
  const businessStores = useMemo(() => {
    return stores.filter((store) => {
      if (store.businessId !== businessId) return false;
  ...
  }, [stores, businessId, searchQuery, filterStatus]);
  ...
  // Handler per entrare come reseller_viewer
  const handleViewStore = (store: Store) => {
    // Crea un operatore temporaneo con ruolo reseller_viewer
    const viewerOperator = {
      id: `viewer-${currentUser?.data.id}-${store.id}`,
      storeId: store.id,
      businessId: store.businessId,
      name: currentUser?.data.adminName || 'Reseller',
      email: currentUser?.data.email || '',
      role: 'reseller_viewer' as const,
      pin: '0000',
      active: true,
      createdAt: new Date(),
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Business non trovato</h3>
          <button
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-lg px-6 py-3">
              <p className="text-indigo-100 text-sm mb-1">Totale Negozi</p>
              <p className="text-3xl font-bold">{businessStores.length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="flex items-start">
            <Eye className="w-5 h-5 text-orange-600 mr-3 mt-0.5 flex-shrink-0" />
            <div>
              <h3 className="font-semibold text-orange-900 mb-1">Modalità Visualizzazione Reseller</h3>
              <p className="text-sm text-orange-700">
                Puoi visualizzare le impostazioni, i prodotti e i clienti di ogni negozio, ma non potrai effettuare vendite.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              {searchQuery || filterStatus !== 'all'
                ...
                : 'Questo cliente non ha ancora creato punti vendita'}
            </p>
          </div>
        )}
      </div>
